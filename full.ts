...
import { addNonIndexCache } from "./indexable.ts";
...
export abstract class TypedVec<in out T, in out A extends TypedArray<T, A>>
  ...
  /**
   * Provides implementation for `index in obj`.
  ...
  /**
   * Provides implementation for `obj[index]`.
   * @param index an index to the array.
  ...
  /**
   * Provides implementation for `obj[index] = value`.
   * @param index an index to the array.
   * @param value the value to set.
   */
  [setElement](index: number, value: T): void {
    if (!Object.is(index, -0) && index < this.#length) {
  ...
  /**
   * Provides implementation for `for (let index in obj)` regarding array indices.
   *
...
for (const key of Object.getOwnPropertyNames(TypedVec.prototype)) {
  addNonIndexCache(key);
}

for (const key of Object.getOwnPropertyNames(Int8Vec.prototype)) {
  addNonIndexCache(key);
}
